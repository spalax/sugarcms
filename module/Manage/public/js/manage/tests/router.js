define(["doh", "dojo/hash", "../router"], function(doh, hash, router){
	var count = 0,
		handle, foo;
	...
	doh.register("tests.router", [
		...
		},
		{
			...
			runTest: function(t){
				...
				router.register("/stop", "/me", function(){ test += "A"; });
				router.register("/stop/me", "/immediate", function(){ test += "B"; });

				router.register("/stop/me/immediate", "/propagation", function(e){
					e.stopImmediatePropagation();
					test += "C";
				});

                router.register("/stop/me/immediate/propagation", "/one", function(){ test += "D"; });
                router.register("/stop/me/immediate/propagation/one", "/more", function(){ test += "E"; });

				router.go("/stop/me/immediate/propagation/one/more");
